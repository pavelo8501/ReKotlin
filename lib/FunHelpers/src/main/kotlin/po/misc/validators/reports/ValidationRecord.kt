...
class ValidationRecord internal constructor(
    ...
    companion object: PrintableCompanion<ValidationRecord>({ValidationRecord::class}){
        ...
        fun fail(container: ValidationContainerBase<*, *>, checkName: String, message: String):ValidationRecord{
          return ValidationRecord(container.validator.validatingCTXName, checkName, CheckStatus.FAILED).setMessage(message)
        }
        fun fail(container: ValidationContainerBase<*, *>, checkName: String, th: Throwable):ValidationRecord{
            return ValidationRecord(container.validator.validatingCTXName, checkName, CheckStatus.FAILED).setMessage(th.message.toString())
        }
    }
}