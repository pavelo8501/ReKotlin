...
class DeferredResultContainer<T: Any>(
    ...
    override val isLambdaProvided: Boolean = true

    override fun invokeLambda():T {
        return resultLambda.invoke()
    }
    ...
    companion object{
        fun <T: Any>  create(creator: CtxId,  resultLambda: ()->T):FunContainer<T>{
...
class PromiseResultContainer<T: Any>(
    ctx: CtxId,
    var resultLambda:(()-> T)?,
): FunContainer<T>(ctx) {
    ...
    companion object{
        fun <T: Any>  create(creator: CtxId,  resultLambda: (()->T)? = null):PromiseResultContainer<T>{
            return  PromiseResultContainer(creator, resultLambda)
        }
    }
}
