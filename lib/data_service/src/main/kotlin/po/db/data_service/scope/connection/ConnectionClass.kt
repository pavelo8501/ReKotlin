...
class ConnectionClass(
    ...
    fun <DATA : DataModel, ENTITY: LongEntity>launchSequence(pack : SequencePack<DATA, ENTITY>){
        coroutineEmitter.dispatch(pack.name){waypoint->
            println("Start  sequence ${pack.name} on waypoint: $waypoint")
            val fn = pack.fn
            val result = pack.context.fn()
            println("Executing sequence ${pack.name} on waypoint: $waypoint")
