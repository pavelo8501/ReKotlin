...
data class ServiceMetadata<DATA_MODEL : DTOMarker, ENTITY : LongEntity>(
...
class ServiceRegistry {
    ...
    fun <DATA_MODEL : DTOMarker, ENTITY : LongEntity> registerService(
    ...
    fun <DATA_MODEL : DTOMarker, ENTITY : LongEntity> getService(
        key: ServiceUniqueKey,
        dtoClass: KClass<DATA_MODEL>,
        entityClass: KClass<ENTITY>
    ): ServiceContext<DATA_MODEL, ENTITY>? {
        val metadata = serviceRegistry[key] ?: return null
        // Validate that the requested classes match the stored classes
