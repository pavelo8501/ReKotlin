...
sealed class SequenceHandlerBase<DTO, D, E>(
    ...
    private var switchQueryProviders : MutableMap<String, () -> SwitchQuery<*,*,*>> = mutableMapOf()
    internal fun getSwitchQuery(handlerName: String):SwitchQuery<*,*,*>{
        val switchQueryProvider = switchQueryProviders[handlerName]
        return  switchQueryProvider?.run {
            invoke()
        }?: throw OperationsException("SwitchQuery requested but switchQueryProvider is null", ExceptionCode.VALUE_IS_NULL)
    }
    internal fun provideSwitchQueryProvider(handlerName: String,  provider : () -> SwitchQuery<*,*,*>){
        switchQueryProviders[handlerName] = provider
    }
}
              ...
              F_DTO : ModelDTO,FD : DataModel, FE: LongEntity
{
    ...
    suspend fun launch(session: AuthorizedSession): ResultList<DTO, D, E> {
        ...
        return handlerDelegate.switchLambda.invoke(newSequenceContext, this)
    }
}
