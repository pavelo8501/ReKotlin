...
abstract class SequenceHandlerAbstraction2<DTO>(
    ...
    private val sequences =
        mutableMapOf<String, SequencePack2<DTO>>()

    internal fun getStockSequence(key: String = "0"): SequencePack2<DTO>{
    ...
    fun addSequence(sequence: SequencePack2<DTO>){
        sequences[thisKey] = sequence
    }

    suspend fun execute(params: Map<String, String>, key: String = "0"): Deferred<List<DataModel>>{
    ...
    suspend fun execute(inputList : List<DataModel>, key: String = "0"): Deferred<List<DataModel>> {
        getStockSequence(key).let {
    ...
    suspend fun execute(params: Map<String, String>, inputList : List<DataModel>, key: String = "0"): Deferred<List<DataModel>>{
        getStockSequence(key).let {
            it.saveParams(params)
            it.saveInputList(inputList)
            return it.serviceClass.launchSequence(it)
    ...
    suspend fun execute(key: String = "0"): Deferred<List<DataModel>>{
        getStockSequence(key).let {
            it.saveParams(emptyMap())
            it.saveInputList(emptyList())
            return it.serviceClass.launchSequence(it)
