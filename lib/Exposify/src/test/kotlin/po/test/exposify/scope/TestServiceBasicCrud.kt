...
class TestServiceBasicCrud : DatabaseTest() {
    ...
    fun `referenced property binding`(){
        ...
        connectionContext?.let {connection->
            connection.service(TestUserDTO.Companion, TableCreateMode.FORCE_RECREATE) {
            ...
            connection.service(TestPageDTO.Companion, TableCreateMode.FORCE_RECREATE) {
    ...
    fun `postgres serializable classes`(){
        ...
        connectionContext?.let {connection->
            ...
            connection.service<TestPageDTO, TestPage>(TestPageDTO.Companion, TableCreateMode.FORCE_RECREATE) {
                ...
                assertEquals(
                    originalPages.pageClasses.count(),
                    updatedPageData.pageClasses.count(),
                    "ClassList count mismatch"
                )
                assertAll(
                ...
                assertEquals(
                    originalPages.pageClasses.count(),
                    selectedPagePageClasses.count(),
                    "PageClasses  count mismatch"
                )
                ...
                assertAll(
                    { assertEquals(1, firstPageClassRecord.key, "Page classList key mismatch in selection") },
                    {
                        assertEquals(
                            "class_1",
                            firstPageClassRecord.value,
                            "Page classList value mismatch in selection"
                        )
                    }
                )
            }
    ...
    fun `updates and selects DTO relations`(){
        ...
        var assignedUserId : Long = 0
        connectionContext?.let {connection->
            connection.service<TestUserDTO, TestUser>(TestUserDTO.Companion, TableCreateMode.CREATE) {
                val userData =  update(user).getData()
                assignedUserId = userData.id
            }
    ...
    fun `updates and selects with conditions`(){
        ...
        var assignedUserId : Long = 0
        connectionContext?.let { connection ->
            connection.service(TestUserDTO.Companion, TableCreateMode.CREATE) {
                val userData = update(user).getData()
                assignedUserId = userData.id
            }
