...
interface TrueHelper{
    ...
    fun formatSystemMsg(prefix: String, message: String, severityLevel: SeverityLevel): String {
        var result =  makeOfColour(ColourEnum.BLUE, prefix)
        return when(severityLevel){
            SeverityLevel.INFO ->{
                "$result ->  ${severityLevel.toString()} $message"
            }
            SeverityLevel.WARNING ->{
                "$result ->  ${severityLevel.toString()} ${makeOfColour(ColourEnum.YELLOW, message)}"
            }
            SeverityLevel.EXCEPTION ->{
                "$result ->  ${severityLevel.toString()} ${makeOfColour(ColourEnum.RED, message)}"
            }
        }
    }
    ...
    fun taskPrefix(task: ResultantTask): String{
        ...
        if(task.nestingLevel == 0){
            template = makeOfColour(ColourEnum.BLUE, "[${task.taskName}| Root Task | $currentTime @ ${task.taskName}]")
        }else{
            template = makeOfColour(ColourEnum.BLUE,
                withIndention(
                    "[${task.taskName}| Nesting Level ${task.nestingLevel} | $currentTime @ ${task.taskName}]",
                    task.nestingLevel)
            )
        }
        return template
